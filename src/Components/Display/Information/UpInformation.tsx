...
const UpInformation: React.FunctionComponent<UpInformationProps> = props => {
  const { iconName, iconSize, iconColor, title, content, action, children } = props;

  const contentClassStyle = classnames(
    'up-box-content',
    style({
      ...contentStyle,
      ...getCustomStyle('content', props),
    })
  );
  ...
  return (
    <UpBox
      className={classnames(
        ...
        style({
          ...boxWrapperStyle,
          ...getCustomStyle('informationWrapper', props),
        })
      )}
    >
      {title && iconName && (
      ...
      <UpBox
        ...
        className={classnames(
          ...
          style({
            ...getCustomStyle('contentWrapper', props),
            ...contentWrapperStyle,
          })
        )}
      >
        {content && (
          <UpBox flexDirection={'column'} justifyContent={'flex-start'} alignItems={'flex-start'}>
            <p className={contentClassStyle}>{content}</p>
            {children && <p className={contentClassStyle}>{children}</p>}
          </UpBox>
        )}
        {action && (
          <UpBox
