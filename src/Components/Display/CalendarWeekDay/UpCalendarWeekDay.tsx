...
export default class UpCalendarWeekDay extends React.Component<UpCalendarWeekDayProps, UpCalendarWeekDayState> {
  ...
  render() {
    const t = this.getCurrentWeek();
    moment.locale('fr');

    const dateNow = new Date();
    const dayofWeek = dateNow.getDay();
    const month = moment.months(dateNow.getMonth());

    const week = t.map((v, i) => {
    ...
    return (
      <div>
        <div className="titreCalendrierHome col-md-12">
          ...
          <div id="moisFR" className="moisTitre col-md-10 bg-blue">
            {month}
          </div>
        </div>
...
class CalendarDay extends React.Component<CalendarDayProps, CalendarDayState> {
  ...
  render() {
    const styleJour: React.CSSProperties = {
    ...
    return (
      <div style={styleJour} onClick={this.onClick}>
...
export class DayDetail extends React.Component<DayDetailProps, DayDetailState> {
  ...
  render() {
    const styleRemovePadding = {
      //   padding :0
    };

    let hourSegement = new Date(
    ...
    for (let i = 0; i < 25; i++) {
      ...
      const eventData = this.getEventIn(hourSegement);
  ...
  getEventIn = (inside: Date) => {
    const mDate = moment(inside);
    const data = this.props.event.filter(v => {
