...
export function calculateDimensions({
  ...
  const itemTimeEnd = _get(item, keys.itemTimeEndKey);
...
export function stack(items, groupOrders, lineHeight, headerHeight, force) {
  ...
  const groupedItems = getGroupedItems(items, groupOrders);
  ...
  groupedItems.forEach(function (group, index, array) {
    ...
    for (i = 0, iMax = group.length; i < iMax; i++) {
      ...
      if (item.dimensions.stack && item.dimensions.top === null) {
        ...
        do {
          collidingItem = null;
          for (let j = 0, jj = group.length; j < jj; j++) {
            const other = group[j];
            if (
              other.top !== null &&
              other !== item &&
              other.dimensions.stack &&
              collision(item.dimensions, other.dimensions, item.dimensions.lineHeight)
            ) {
              collidingItem = other;
              break;
            } else {
...
export function nostack(items, groupOrders, lineHeight, headerHeight, force) {
  ...
  groupedItems.forEach(function (group, index, array) {
    ...
    for (i = 0, iMax = group.length; i < iMax; i++) {
      const item = group[i];
      const verticalMargin = (item.dimensions.lineHeight - item.dimensions.height) / 2;
...
export function createGradientPattern(lineHeight, color1, color2, borderColor) {
  if (borderColor) {
    ...
    if (!color2 || color1 === color2) {
      ...
      return `repeating-linear-gradient(to bottom,${color1},${color1} ${lineHeight}px,${color2} ${lineHeight}px,${color2} ${
        lineHeight * 2
      }px)`;
    }
  }
