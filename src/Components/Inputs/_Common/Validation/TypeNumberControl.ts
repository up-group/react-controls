...
export default class TypeNumberControl implements ErrorControl<any> {
  ...
  isValidValue(value: number | string): ErrorControlType<number> {
    ...
    if (isNaN(nbValue)) {
      return {
        hasError: true,
        errorMessage: 'Doit être un nombre',
      };
    } else if (this._isInteger && nbValue % 1 !== 0) {
      return {
        hasError: true,
        errorMessage: 'Doit être un entier',
      };
    } else if (this._minValue > nbValue) {
      return {
        hasError: true,
        errorMessage: "Doit être plus grand que '" + this._minValue + "'",
      };
    } else if (this._maxValue < nbValue) {
      return {
        hasError: true,
        errorMessage: "Doit être plus petit que '" + this._maxValue + "'",
      };
    }
