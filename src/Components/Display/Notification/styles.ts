...
export const colors = (props: UpNotificationProps & WithThemeProps ) : NestedCSSProperties => {
  return { 
      position: 'relative',
      backgroundColor: props.displayMode === 'text' ? 'transparent' : props.theme.colorMap[`${props.intent}`] || props.theme.colorMap.white3,
      ...
      $nest : {
            '& p, & span, & div, & em, & strong' : {
                fontWeight: 400,
                textAlign:'left',
                color: props.displayMode === 'text' ? props.theme.colorMap[`${props.intent}`] : props.theme.colorMap.white1 ||  props.theme.colorMap.darkGray5,
            },
...
export const icon  = (props : UpNotificationProps & WithThemeProps) : NestedCSSProperties => {
  ...
  const iconContainerStyle : NestedCSSProperties = props.intent === 'success' || props.intent ==='error' || props.intent === 'danger' ? {marginLeft:'25px'} : iconWithBorderStyle
  return {
    $nest: {
      "& .up-notification .up-notification-icon-container" : { ...iconContainerStyle },
      "& .up-notification .colored svg, & .up-notification .colored svg path, & .up-notification .colored svg polygon, & .up-notification .colored svg polyline": {
        fill: props.displayMode=== 'text'? props.theme.colorMap[`${props.intent}`] :   props.intent !== 'success' && props.theme.colorMap.white1,
        //margin: "10px",
...
interface AnimarionRef {
  animation ?: any
}
export const getStyles  = (props: UpNotificationProps & AnimarionRef) : string => (
  classnames(style(colors(props)), style(icon(props)),style(text(props)),style(progressBar(props)), style({
    width:'100%',
    padding:'8px',
    borderRadius:'4px',
    boxSizing:'border-box',
    minHeight:'100px',
    borderBottomRightRadius:`${props.duration && '0px !important'}`,
    borderBottomLeftRadius:`${props.duration && '0px !important'}`,
    display:'flex',
    alignItems: 'center',
  }))
);
