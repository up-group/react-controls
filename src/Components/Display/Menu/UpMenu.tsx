...
class UpMenu extends React.Component<UpMenuProps & WithThemeProps, UpMenuState> {
  ...
  render() {
    ...
    const menu = menuItems.map((v, i) => {
      ...
      return <MenuItem onClick={this.props.onClick} key={i} title={v.title} icon={v.icon} uri={v.uri}
                       isSelected={v.isSelected} isVisible={v.isVisible} childMenuItems={v.childMenuItems}
                       isSeparator={v.isSeparator}/>;
    });
    ...
    return <li className={classnames('treeview', {hide, active, hasChildren: hasChilren, separator: isSeparator})}>
      ...
      <a onClick={(e) => {
        ...
        this.setSubMenuPosition(e);
         ...
         href={this.props.uri}
      >
          <UpSvgIcon title={this.props.title} width={22} height={22}
                     iconName={this.props.icon as IconName}></UpSvgIcon>
...
export class SubMenu extends React.Component<SubMenuProps> {
  ...
  render() {
    ...
    const items = this.props.childMenuItems.map((v, i) => {
      return <SubItems key={i} onClick={this.props.onClick} uri={v.uri} title={v.title} isVisible={v.isVisible}
                       isSelected={v.isSelected} icon={v.icon} childMenuItems={v.childMenuItems}/>;
    });
    ...
    return <li className={active + hide}>
      <a onClick={this.onClickA} href={this.props.uri}>
        ...
        <i onClick={this.onClick}
           className={(this.state.active ? 'pe-7s-angle-down' : 'pe-7s-angle-right')}></i>
        }
        <span className={'up-menu-item-title'}>{this.props.title}</span>
      </a>
