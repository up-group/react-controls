...
describe('Tests for UpPanel', () => {
  it('should render title', () => {
    const { container } = renderComponent(
      <UpPanel title={'Mon panel'} />
    );

    expect(
      container.querySelector('.up-panel_header')
    ).toHaveTextContent('Mon panel');
  });
  ...
  it('should render title as react element', () => {
    ...
    expect(screen.getByText('Mon JSX Panel').parentNode).toHaveClass(
      'up-panel_header'
    );
  });
  ...
  it('should render a message body', () => {
    const { container } = renderComponent(
      <UpPanel title={'Mon panel'} message={'Mon panel text'} />
    );

    expect(
      container.querySelector('.up-panel_message')
    ).toHaveTextContent('Mon panel text');
  });

  it('should render footer', () => {
    const { container } = renderComponent(
      <UpPanel footer={'this is the footer'} />
    );

    expect(
      container.querySelector('.up-panel_footer')
    ).toHaveTextContent('this is the footer');
  });

  it('should render footer as react element', () => {
    const { container } = renderComponent(
      <UpPanel footer={<h3>this is the footer</h3>} />
    );

    expect(container.querySelector('.up-panel_footer')).toContainHTML(
  ...
  it('should not show an icone if truthy disableAutoIntentIcon or no icon is provided', () => {
    ...
    expect(
      container.querySelector('.up-panel_message').parentNode
    ).toHaveClass('up-col-24');
  });
