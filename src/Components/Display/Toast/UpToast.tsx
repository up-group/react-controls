...
const UpToast = (props: UpToastProps & WithThemeProps) => {
  ...
  return (
    ...
    >
      {title && (
        <>
          <UpHeading tag={'h4'} margin={'none'} className={classnames(toastTitleStyle, 'up-toast-title')}>
            {title}
          </UpHeading>
          <div className={classnames(buttonStyle, 'up-toast-close')} onClick={handleClose}>
            <UpSvgIcon width={'15px'} height={'15px'} iconName={'close'} />
          </div>
        </>
      )}
