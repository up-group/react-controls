...
const animationKeyframes = (fadeType?: fadeType, fadeDirection?: fadeDirection) =>
  keyframes({
    from: {
...
export const fadeInAnimation = (duration = 1, fadeMode: fadeMode = 'ease') => ({
  animation: `${fadeInKeyframes} ${duration}s ${fadeMode}`,
});

export const fadeOutAnimation = (duration = 1, fadeMode: fadeMode = 'ease') => ({
