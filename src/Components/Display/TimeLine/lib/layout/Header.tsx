...
import { iterateTimes, getNextUnit } from '../utils';
...
export default class Header extends React.Component<any, any> {
  ...
  setComponentTop() {
    const viewportOffset = (this.refs as any).header.getBoundingClientRect();
    this.setState({
  ...
  touchEnd = e => {
    ...
    if (changedTouches) {
      const elem = document.elementFromPoint(changedTouches.pageX, changedTouches.pageY);
  ...
  render() {
    ...
    iterateTimes(canvasTimeStart, canvasTimeEnd, minUnit, timeSteps, (time, nextTime) => {
      ...
      const leftCorrect =
        fixedHeader === 'fixed' ? Math.round((canvasTimeStart - visibleTimeStart) * ratio) - borderWidth + 1 : 0;
