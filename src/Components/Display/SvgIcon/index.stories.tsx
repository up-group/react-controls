...
import logoSvg from './logo-up-square.svg';
...
const Item = ({ color, icon, width, height }) => {
  return (
    <>
      ...
      <dd>
        <UpSvgIcon iconName={icon} color={color} width={width} height={height} onClick={console.log} />
      </dd>
    </>
  );
};
  ...
  return (
    <UpThemeProvider theme={UpDefaultTheme}>
      <UpBox style={{ margin: '40px 30px' }}>
        ...
        <UpBox flexDirection={'row'} flexWrap={true}>
          {MentorNames.sort().map((icon, index) => (
            <dl key={index} style={{ margin: '10px' }}>
              <dt
                style={{
                  color: color,
                  marginBottom: '10px',
              ...
              >{`${icon}`}</dt>
              <dd>
                <UpSvgIcon iconName={icon} color={color} width={48} height={48} />
              </dd>
            </dl>
          ))}
        </UpBox>
            ...
            if (icon != 'sparkles')
              return (
                <dl style={{ margin: '10px' }}>
                  ...
                  >{`${icon}`}</dt>
                  <dd>
                    <UpSvgIconSparkles iconName={icon} color={color} width={48} height={48} />
                  </dd>
                </dl>
              );
          })}
        ...
        <UpBox flexDirection={'row'} flexWrap={true}>
          {IllustrationNames.sort().map((icon, index) => (
            <dl key={index} style={{ margin: '10px' }}>
              <dt
                style={{
                  color: color,
              ...
              >{`${icon}`}</dt>
              <dd>
                <UpSvgIcon iconName={icon} color={color} width={115} height={115} />
              </dd>
            </dl>
          ))}
        </UpBox>
