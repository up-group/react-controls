...
class UpMenu extends React.Component<UpMenuProps & WithThemeProps, UpMenuState> {
  ...
  render() {
    ...
    const menu = menuItems.map((v, i) => {
      if (v.render != null) {
        const e = v.render(v, others, this.state);
    ...
    if (children != null && isFunction(children)) {
      renderChildren = (children as RenderCallback)(this.props, this.state);
    }
    ...
    return (
      <aside
        className={classnames(
          ...
          getMenuStyles({
            ...this.props,
            minified: this.currentMinifiedValue,
          })
        )}
      >
        ...
        {renderChildren && <section className="up-menu-content">{renderChildren}</section>}
        {renderFooter && <section className="up-menu-footer">{renderFooter}</section>}
      </aside>
    );
  }
}
    ...
    return (
      <>
        <ul className="up-sub-menu">
          {this.props.title && <span className={'up-sub-menu-title'}>{this.props.title}</span>}
          {items}
        </ul>
      </>
    );
