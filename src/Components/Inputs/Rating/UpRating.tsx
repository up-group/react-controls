...
class UpRating extends React.PureComponent<RatingProps & WithThemeProps, RatingState> {
  ...
  render() {
    const { className, value, numberOfStars, max, dataFor, theme, onChange, ...rest } = this.props;

    return (
      <div
        className={classnames(RatingWrapperStyle(this.props), className, 'up-rating')}
        data-for={dataFor}
        data-tip={'tooltip'}
        {...rest}
      >
        {Array(numberOfStars)
          .fill(0)
          .map((element, index) => {
            const fillMode = getStarFill(
              index + 1,
              numberOfStars,
              this.state.editedValue != null ? this.state.editedValue : value,
              max
            );
