...
export const headTabArrow = function(colorOnFocus: string) {
  return style({
    color: 'transparent',
    position: 'absolute',
    bottom: toRem(-11),
    ...
    borderColor: colorOnFocus + " transparent",
    borderStyle: 'solid',
    width: 0,
    opacity: 1,
  });
...
export const headTabBorder = function(colorOnFocus: string) {
  return style({
    background:  colorOnFocus,
    display: 'block',
    width: '100%',
    height: '5px',
    position: 'absolute',
...
export const tabItem = (props: TabHeadItemProps): NestedCSSProperties => ({
  ...
  color: props.tabKey === props.selectedTabKey ? props.colorOnFocus : '#4E5B59',
  $nest: {
    '&:first-child': {
      marginLeft: 0,
    },
