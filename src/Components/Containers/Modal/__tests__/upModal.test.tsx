...
describe('Tests for UpModal', () => {
  ...
  it('should add a header seperator by default', () => {
    ...
    expect(container.querySelector('.up-modal_header')).toHaveStyle(
  ...
  it('should remove the header seperator', () => {
    const { container } = renderComponent(
      <UpModal withHeaderSeparator={false} />
    );

    expect(
      container
        .querySelector('.up-modal_header')
        .getAttribute('style')
    ).toBeNull();
  });
  ...
  it('should show a header as string', () => {
    const header = 'Header';
    const { container } = renderComponent(
      <UpModal header={header} />
    );

    expect(container.querySelector('.up-modal_header')).toContainHTML(
      '<h3 class="up-modal_title">Header</h3>'
    );
  });
  ...
  it('should show a header as React element', () => {
    ...
    expect(container.querySelector('.up-modal_header')).toContainHTML(
      '<h4 class="up-modal_title">Other Header</h4>'
    );
  });
  ...
  it('should show a footer', () => {
    const footer = 'Footer';
    const { container } = renderComponent(
      <UpModal footer={footer} />
    );

    expect(container.querySelector('.up-modal_footer')).toContainHTML(
      '<div class="up-modal_footer">Footer</div>'
    );
  });
