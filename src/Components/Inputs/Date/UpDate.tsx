...
class UpDate extends BaseControlComponent<UpDateProps & WithThemeProps, moment.Moment> {
  ...
  onChange = (startDate: moment.Moment, endDate?: moment.Moment) => {
    ...
    if (moment(value, this.props.format, true).isValid()) {
      const isOutsideRange = this.props.isOutsideRange || this.defaultIsOutsideRange;
  ...
  resetDateInputValue = (force = false) => {
    ...
    >
      <div style={{ width: '30%' }}>
        <UpSelect
          value={{
            id: month.month(),
            text: this.formatMonth(month.month()),
          }}
  ...
  defaultIsOutsideRange = (day: Date | moment.Moment): boolean => {
    ...
    return (
      (maxDate && _day.toDate().setHours(0, 0, 0, 0) > maxDate) ||
      (minDate && _day.toDate().setHours(0, 0, 0, 0) < minDate)
    );
  };

  renderControl() {
    const { disabled, minDate, maxDate, floatingLabel, placeholder, iconPosition } = this.props;
