...
export default class CanvasState {
    ...
    canvas.addEventListener(
      ...
      function (e) {
        if (myState.dragging) {
          ...
          const mouse = myState.getMouse(e);
          // Add temp shape
          const _h = Math.abs(mouse.y - myState.drawingoffy);
          const _w = Math.abs(mouse.x - myState.drawingoffx);
          const _shape = new Shape(myState.drawingoffx, myState.drawingoffy, _w, _h, 'rgba(0,255,0,.6)');
          _shape.temp = true;
    ...
    canvas.addEventListener(
      ...
      function (e) {
        if (myState.drawing === true) {
          ...
          const _w = Math.abs(mouse.x - myState.drawingoffx);
          if (_w > 120 && _h > 17) {
            const _shape = new Shape(myState.drawingoffx, myState.drawingoffy, _w, _h, mouse.y, false);
            myState.addShape(_shape);
          } else {
  ...
  cropShape = (shape: Shape) => {
    ...
    try {
      crop_canvas
        .getContext('2d')
        .drawImage(this.imageObj, left - this.border, top - this.border, width, height, 0, 0, width, height);
  ...
  drawImage = () => {
    const WIDTH = this.imageObj.width + 2 * this.border;
    const HEIGHT = this.imageObj.height + 2 * this.border;
  ...
  getMouse = e => {
    ...
    const my = e.pageY + scrollY - offsetY;
