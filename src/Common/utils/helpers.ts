...
export function getFontSizeNumber(size: number | string): number {
  ...
  const regex = /([0-9]*\.?[0-9]*)(.*)/i.exec(size);

  if (isNullOrUndef(regex) || isNullOrUndef(regex[1])) {
    return 0;
  }

  const sizeParsed: number = parseFloat(regex[1]);
  const unite: string = regex[2];
...
export function arrayIsIdentical(array1: any[], array2: any[]): boolean {
  ...
  for (let cpt = 0; cpt < array1.length; cpt++) {
...
export function jourDuMois(mois: number, annee: number, moisBase0 = false): number {
  const fevrier: number = isBissextile(annee) ? 29 : 28;
  switch (mois) {
    case 0:
...
export function incrementJour(date: Date, increment: number): Date {
  ...
  let mois: number = date.getMonth();
  let jour: number = date.getDate() + increment;
  let jdm: number = jourDuMois(mois, annee, true);
...
export function ConvertToDate(date: string): Date {
  ...
  date = date.trim();
  const jourMoisAnnee: number[] = [];
  let nbPas = 0;

  for (let idx = 0; idx < date.length; idx++) {
    const car: string = date[idx];
  ...
  if (jourMoisAnnee[2] <= 99) {
    ...
    const anneeLimite: number = anneeEnCours % 100;
