...
class UpNumber extends BaseControlComponent<UpNumberProps, number | string> {
  ...
  constructor(p, c) {
    ...
    this._validationManager.addControl(
      new TypeNumberControl(this.props.decimalPlace === 0, this.props.min, this.props.max)
    );
  }
  ...
  applyDecimalPlace = (value: string) => {
    ...
    if (this.props.decimalPlace != null) {
      ...
      const _value: number = parseFloat(value.replace(',', '.'));
  ...
  handleNumericChange = (event: React.ChangeEvent<any>, valueAsString: string) => {
    if (this.isValueMatched(valueAsString)) {
      ...
      if (
        (this.props.max && parseFloat(currentValue)) > this.props.max ||
        (this.props.min && parseFloat(currentValue)) < this.props.min
      ) {
        return null;
      }
  ...
  renderControl() {
    const { className, isRequired, theme, readonly, tooltip, placeholder, name, autoFocus, floatingLabel } = this.props;
