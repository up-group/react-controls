      ...
      <div className="">
        <div>
          ...
          <div className={styleContentWrapper}>
            <section className={styleContent}>{this.props.children}</section>
          </div>
        </div>
      </div>
    );
...
export class SubItems extends React.Component<SubItemsProps, SubItemsState> {
  ...
  get isMenuSelected() {
    return (
      this.props.isSelected || this.props.selectedBranchId.substr(0, this.props.branchId.length) === this.props.branchId
    );
  }
        ...
        <div className={liLine}>
          {this.anyChild ? (
            <i
              ...
              className={
                inliei + ' ' + (this.state.active || this.isMenuSelected ? 'pe-7s-angle-down' : 'pe-7s-angle-right')
              }
            ></i>
          ) : null}
