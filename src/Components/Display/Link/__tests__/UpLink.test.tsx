...
describe('Tests for UpLink', () => {
  ...
  it('should add correct href', () => {
    renderComponent(<UpLink label={'My anchor'} href="https://example.com" />);

    expect(screen.getByText('My anchor').getAttribute('href')).toBe('https://example.com');
  });

  it('should add correct target', () => {
    renderComponent(<UpLink label={'My anchor'} target="_blank" />);

    expect(screen.getByText('My anchor').getAttribute('target')).toBe('_blank');
  });
  ...
  it('should add class to anchor', () => {
    ...
    expect(screen.getByText('My anchor').className.length).toBeGreaterThan(0);
  });

  it('should add tooltips if dataFor is provided', () => {
    renderComponent(<UpLink label={'My anchor'} dataFor={'anchor'} />);

    expect(screen.getByText('My anchor').getAttribute('data-tip')).toBe('tooltip');
    expect(screen.getByText('My anchor').getAttribute('data-for')).toBe('anchor');
  });
});
