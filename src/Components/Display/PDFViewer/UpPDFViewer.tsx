...
class UpPDFViewer extends React.PureComponent<UpPDFViewerProps & WithThemeProps, UpPDFViewerState> {
  ...
  setDivSize = () => {
    if (this.props.width) {
      ...
      this.setState({
        wrapperWidth: this.pdfWrapper.getBoundingClientRect().width,
      });
    }
  };
  ...
  render() {
    ...
    if (isProcessingPrint === true) {
      return <UpLoadingIndicator message="Chargement en cours..." isLoading={isProcessingPrint} />;
    }
    ...
    return (
      ...
      >
        <Document
          ...
          noData={''}
        >
          {this.props.showAllPages && this.state.pagesNumber && this.displayAllPages(this.state.pagesNumber)}
