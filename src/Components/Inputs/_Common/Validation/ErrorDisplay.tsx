...
const getErrorDisplayStyle = (props: ErrorDisplayProps & WithThemeProps) =>
  style({
    position: 'relative',
...
export default class ErrorDisplay extends React.Component<ErrorDisplayProps & WithThemeProps> {
  ...
  render() {
    ...
    return (
      <div className={getErrorDisplayStyle(this.props)}>
        {this.props.children}
        {this.props.showError === true && this.props.hasError === true ? <Error {...this.props} /> : null}
      </div>
    );
  }
}
...
const ErrorTooltip = (props: ErrorDisplayProps) => {
  return (
    <div className={'up-wrapper-error-tooltip'}>
