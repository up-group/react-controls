...
class UpDropFile extends React.Component<UpDropFileProps & WithThemeProps, UpDropFileState> {
  ...
  static defaultProps: Partial<UpDropFileProps> & WithThemeProps = {
    ...
    allowedExtensionsErrorMessage: (allowedExtension: string[]) =>
      `Les formats de fichier autorisés sont : ${allowedExtension.join(', ')}`,
    showOptions: true,
  ...
  updateWrapperHeight = () => {
    if (this.ratio != null && this.wrapperUpDropFile != null) {
      this.setState({
        width: this.value && this.props.showPreview ? this.wrapperUpDropFile.clientWidth : null,
        height: this.value && this.props.showPreview ? this.wrapperUpDropFile.clientWidth / this.ratio : null,
      });
    }
  };
      ...
      <>
          ...
          {!isEmpty(this.props.label) && (
            <UpLabel text={`${this.props.label} ${this.props.required ? ' *' : ''}`}>
              {this.props.onMouseOver && (
                <span className={classnames(iconTitleStyle, 'icon-info')} onClick={this.props.onMouseOver} />
              )}
            </UpLabel>
          )}
            ...
            >
              {!isFileSelected && this.props.displaySelectFile && (
                <div style={{ padding: '3rem' }}>
                  ...
                  <div>
                    <UpButton intent="primary" onClick={e => this.onZoneClick(e)}>
              ...
              {isFileSelected && !this.props.showPreview && (
                <UpNotification intent={'info'} displayMode={'text'}>
                  <UpBox onClick={e => e.stopPropagation()} flexDirection={'column'}>
                    <UpParagraph>
                    ...
                    <>
                      {this.props.disabled !== true && (
                        <div onClick={e => e.stopPropagation()}>
                          <UpTooltip content={`${this.props.deleteFileLabel} ${this.fileName || ''}`} place={'top'}>
                            <UpSvgIcon
                              iconName={'trash-can'}
                      ...
                      {this.props.enableCrop && this.value && isFileImage(this.value.name) && (
                        <div onClick={e => e.stopPropagation()}>
                          <UpTooltip content={this.props.resizeImageLabel} place={'top'}>
                            <UpSvgIcon
                              iconName={'crop'}
                      ...
                      {this.value && (
                        <div onClick={e => e.stopPropagation()}>
                          <UpTooltip content={`${this.props.openFileLabel} ${this.fileName}`} place={'top'}>
                            <UpSvgIcon
                              iconName={'read'}
          ...
          {isFileSelected && this.fileName && (
            <div className={wrapperFileNameStyle(this.props)}>
              ...
              <span style={{ cursor: 'pointer' }}>
                <UpSvgIcon width={15} height={15} iconName={'trash-can'} color="#4E5B59" onClick={this.deleteFile} />
              </span>
            </div>
          )}
        ...
        {this.props.enableCrop && this.state.showModal && this.value != null && (
          <UpModal onClose={this.closeModal} showModal={this.state.showModal}>
            <UpButton actionType={'save'} intent={'primary'} onClick={this.uploadFile.bind(this)}>
              Sauvegarder
            </UpButton>
            <Cropper
              ref={cropper => {
