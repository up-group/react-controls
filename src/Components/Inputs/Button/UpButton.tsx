...
export const ActionIconMap = new Dictionary<ActionType, IconName>([]);
...
export class BaseButton extends React.Component<UpButtonStyledProps> {
  render() {
    const { children, className, onClick, dataFor, width, isProcessing, type, disabled, dropDown, name } = this.props;
    ...
    const MainButton = (
      <button
        ...
        className={classnames(
          'up-btn',
          dropDown !== 'none' && children ? 'up-btn-drop-down' : '',
...
class UpButton extends React.Component<UpButtonProps, UpButtonState> {
  ...
  getDropDownStyles = () => {
    ...
    if (
      ...
      this.dropDownContainer.getBoundingClientRect().right + this.dropDownContainer.offsetWidth + 20 >= viewportWidth
    ) {
      styles.right = '10px';
  ...
  render() {
    ...
    const handleClickProps = this.props.type === 'submit' ? {} : { onClick: this.handleClick };
      ...
      return (
        <div style={{ display: 'flex' }}>
          <BaseButton
            ...
            {...renderButtonProps}
          >
            {children != null && <span className={'up-btn-label'}>{children}</span>}
          </BaseButton>
          {!children && this.props.dropDown != 'none' && this.getValue('isToggled') && (
            <span className="up-btn-missing-border" />
          )}
        </div>
      );
    };
    ...
    return (
      <div
        ...
        className={classnames('up-btn-wrapper', getWrapperStyles(this.props), this.props.className)}
      >
        {tooltip === null ? (
          <RenderButton />
        ) : (
