...
export const RequiredMark = ({ markStyle }) => <UpLigne className={style(markStyle)}>*</UpLigne>;

const BaseInput: React.FunctionComponent<UpInputStyledProps & WithThemeProps & UpInputProps> = (
  props: UpInputStyledProps & WithThemeProps & UpInputProps
) => {
  const {
    name,
    autocomplete,
  ...
  if (iconName) {
    icon = <SvgIcon iconName={iconName} width={size} height={size} color={props.color} />;
  }

  if (showClearIcon) {
    RightIcon = <SvgIcon iconName="clear" width={size} height={size} className={'clear-icon'} onClick={onClear} />;
  }
    ...
    <div className={classnames(getStyles(props), className)}>
      ...
      >
        ...
        {iconPosition === 'right' && props.floatingLabel && (
          <label className={className} htmlFor={id}>
            {props.floatingLabel}
            {props.isRequired ? <UpLigne className={style(starStyle)}>*</UpLigne> : ''}
          </label>
        )}
...
class UpInput extends BaseControlComponent<UpInputProps, any> {
  ...
  getValue(event: any) {
    return event == null ? null : event.target != null ? event.target.value : event;
  }
