    ...
    <div className={'up-calendar'} style={{ width: '100%' }}>
      ...
      {referenceElement && tooltipEvent && (
        <div
          ...
          {...(attributes?.popper || {})}
        >
          <div className="up-calendar__tooltip__arrow" ref={setArrowElement} style={styles.arrow} />
          {props.renderTooltipHeader && (
            <div className="up-calendar__tooltip__header">{renderTooltipHeader(tooltipEvent)}</div>
          )}
          {props.renderTooltipContent && (
            <div className="up-calendar__tooltip__content">{renderTooltipContent(tooltipEvent)}</div>
          )}
        </div>
      )}
    </div>
