...
export default class UpCalendarTimeline extends React.Component<any, any> {
  ...
  componentDidMount() {
    ...
    (this.refs as any).scrollComponent.addEventListener('touchstart', this.touchStart);
    (this.refs as any).scrollComponent.addEventListener('touchmove', this.touchMove);
    (this.refs as any).scrollComponent.addEventListener('touchend', this.touchEnd);
  }

  componentWillUnmount() {
    window.removeEventListener('resize', this.resizeEventListener);
    (this.refs as any).scrollComponent.removeEventListener('touchstart', this.touchStart);
    (this.refs as any).scrollComponent.removeEventListener('touchmove', this.touchMove);
    (this.refs as any).scrollComponent.removeEventListener('touchend', this.touchEnd);
  }
  ...
  touchMove = e => {
    ...
    if (this.lastTouchDistance && e.touches.length === 2) {
      ...
      if (deltaX !== 0 && moveX) {
        (this.refs as any).scrollComponent.scrollLeft -= deltaX;
      }
      if (moveY) {
  ...
  resize() {
    ...
    const { width: containerWidth, top: containerTop } = (this.refs as any).container.getBoundingClientRect();
    const width = containerWidth - this.props.sidebarWidth;
    ...
    (this.refs as any).scrollComponent.scrollLeft = width;
  }

  onScroll = () => {
    const scrollComponent = (this.refs as any).scrollComponent;
  ...
  updateScrollCanvas = (visibleTimeStart, visibleTimeEnd, forceUpdateDimensions, updatedItems, updatedGroups) => {
    ...
    if (canKeepCanvas) {
      ...
      if ((this.refs as any).scrollComponent.scrollLeft !== newScrollLeft) {
        resetCanvas = true;
      }
    } else {
      resetCanvas = true;
    ...
    if (resetCanvas) {
      ...
      (this.refs as any).scrollComponent.scrollLeft = this.state.width;
  ...
  onWheel = e => {
    ...
    if (e.ctrlKey) {
      ...
      if (this.props.fixedHeader === 'fixed') {
        ...
        if (e.deltaX !== 0) {
          if (!traditionalZoom) {
            (this.refs as any).scrollComponent.scrollLeft += e.deltaX;
          }
        }
        if (e.deltaY !== 0) {
  ...
  showPeriod = (from, unit) => {
    ...
    let visibleTimeEnd: any = moment(from).add(1, unit).valueOf();
  ...
  handleMouseMove = e => {
    if (this.state.isDragging && !this.state.draggingItem && !this.state.resizingItem) {
      (this.refs as any).scrollComponent.scrollLeft += this.state.dragLastPosition - e.pageX;
  ...
  stackItems(items?, groups?, canvasTimeStart?, visibleTimeStart?, visibleTimeEnd?, width?, full?): any {
    const {
      keys,
      dragSnap,
      lineHeight,
    ...
    const dimensionItems = visibleItems
      .map(item => {
        return {
    ...
    const stackingMethod: any = stackItems ? stack : nostack;

    const { height, groupHeights, groupTops } = stackingMethod(dimensionItems, groupOrders, lineHeight, headerHeight);
  ...
  handleDoubleClick = e => {
    ...
    const boundingRect = (this.refs as any).scrollComponent.getBoundingClientRect();
  ...
  render() {
    ...
    const { draggingItem, resizingItem, isDragging, width, visibleTimeStart, visibleTimeEnd, canvasTimeStart } =
      this.state;
    ...
    const outerComponentStyle: React.CSSProperties = {
      height: `${height}px`,
      display: 'block',
      overflow: 'hidden',
    };
    ...
    const canvasComponentStyle: React.CSSProperties = {
